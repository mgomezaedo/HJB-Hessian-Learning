%MAIN_REGRESS  Entry point: fit the 19-D Allen-Cahn value function, report error.
%   Loads the datasets produced by generation/allencahn_19d, builds the chosen
...
% ============================ CONFIG ============================
% Folder with the .mat datasets written by generation/allencahn_19d:
data_dir = fullfile(here, '..', '..', 'generation', 'allencahn_19d', 'data');
train_file = 'train_ac19d.mat';
val_file   = 'val_ac19d.mat';

% --- basis choice ---
basis_type = 'HC';   % 'HC' = hyperbolic cross, 'enriched' = HC + total-degree
hc_level   = 4;      % HC level
td_level   = 5;      % total-degree level (only used if basis_type='enriched')
...
margin = 0.0;        % basis domain extension beyond the box (0 -> exactly [-1,1])
% ================================================================
...
d      = Dtr.meta.d;
a_half = (Dtr.meta.ub - Dtr.meta.lb) / 2;     % box half-widths (per dim)
a_vec  = a_half * (1 + margin);               % basis half-widths
...
% save(fullfile(here,'coeffs_ac19d.mat'), 'theta', 'NN', 'a_vec', 'ord', 'basis_type');
